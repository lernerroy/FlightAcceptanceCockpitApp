		...
		onTheDetailPage : {
			...
			assertions : {
				...
				theLineItemsListShouldHaveTheCorrectNumberOfItems : function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet : "FlightSegmentItemSetAC",
						success : function (aEntitySet) {
							...
							return this.waitFor({
								...
								check: function (oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("Aufnr");

									var iLength = aEntitySet.filter(function (oLineItem) {
										return oLineItem.Aufnr === sObjectID;
									}).length;
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							return this.waitFor({
								id : "lineItemsTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name: "text", value: "<FlightSegmentItemSetACPlural> (" + iNumberOfItems + ")"}),
								success : function () {
